...
%               numberOfLabel       : the cell(s) to be extracted
...
% numberOfLabel determines WHICH ROI is selected. If the number does not exist or is
% passed as empty, then segment ALL possible labels
if ~exist('numberOfLabel','var')
    numberOfLabel               = [];
    %    numberOfLabel              =  1;
end
if isempty(numberOfLabel)
    numberOfLabel               = 1:levs;
end
...
if numel(numberOfLabel)>1
    % iterate over the number of labels
    for counterLabel=1:numel(numberOfLabel)
        segmentHelaROI(inputFolder,IndividualHelaLabels,numberOfLabel(counterLabel),inputSlice);
