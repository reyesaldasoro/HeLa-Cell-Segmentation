...
% This is a more detailed description of how to process a whole stack of images that
% contain one HeLa cell, that is one Region of Interest (ROI) consisting of
% approximately 300 slices each with dimensions 2,000 x 2,000 pixels. These can be
% saved as single slices inside a folder, or as a multi-slice tiff. Let's assume that
% you have several multi-slice tiffs. First, read all the files existing in that
% folder

dir0                    = dir('*.tiff');

%%
% It is practical to save this in a directory as later on you can select the stack
% you want to process by choosing one of the files saved in the directory, e.g.

currentSet              = 12;
currentName             = dir0(currentSet).name;
disp(currentName)
%%
% Once this set has been selected, we can read the information of the file to know
% how many slices are saved in this file:
...
%%
% Notice that we filtered the image with a Gaussian Low Pass filter for display
% purposes as the original images are grainy. Notice also how we displayed the name
% of the file, with the number of the set and the current slice.

%%
% We can now segment the nucleus and the background of the current image. We can do
...
%%
% Notice that we stored the result on a 3D matrix, with the level given by the number
% of the central slice. This is done so that we can later on save  all the slices and
% create a 3D volume.
%%
% The segmentation on its own is not as revealing as when it is combined with the
% original data. For that we can overlay on the original image, using 3 levels for
% the RGB components:
...
%%
% Notice first, the graininess of the original image, and also that, as the original
% image was a uint8, we had to convert the results prior to combining them.

%% Multiple slices
% Once the central slice has been segmented, we can use that result as an input
% argument to the function and do this iteratively for all the slices of the set. We
% will go up first, and then down. For curiosity, the time it takes to process all
% the slices will be calculated with tic-toc
tic
...
% Finally, we would like to visualise the results, there are several ways to do this,
% one is to create a video or animated GIF changing the slices, which will be
% described in a separate section. Here we will display the 3D cell as a rendered
% surface. For this, we need to first obtain the dimensions of the cell
